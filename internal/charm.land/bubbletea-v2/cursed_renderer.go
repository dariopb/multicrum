...
func (s *cursedRenderer) start() {
	...
	if s.lastView.KeyboardEnhancements != (KeyboardEnhancements{}) {
...
func (s *cursedRenderer) flush(closing bool) error {
	...
	if s.lastView == nil || view.KeyboardEnhancements != s.lastView.KeyboardEnhancements ||
		...
		if view.KeyboardEnhancements != (KeyboardEnhancements{}) {
			_, _ = s.scr.WriteString(ansi.SetModifyOtherKeys2)
		...
		} else if s.lastView != nil && s.lastView.KeyboardEnhancements != (KeyboardEnhancements{}) {
			_, _ = s.scr.WriteString(ansi.ResetModifyOtherKeys)
